...
public class SessionMergedListener  implements Consumer<Event<WorkflowStatusEvent>> {

    private final NrgEventService eventService;

    @Autowired
    public SessionMergedListener(final EventBus eventBus, final NrgEventService eventService) {
        eventBus.on(type(WorkflowStatusEvent.class), this);
        this.eventService = eventService;
    }

    //*
    // Translate "Merged" workflow event into SessionArchivedEvent for workflow events containing Session type
    //*
    @Override
    public void accept(Event<WorkflowStatusEvent> event) {
        final WorkflowStatusEvent wfsEvent = event.getData();

        if (StringUtils.equals(wfsEvent.getEventId(), "Merged") && wfsEvent.getEntityType().contains("Session")) {
            try {
            ...
            log.error("HELLO in session mergedlistener - this was not a Merged event. it was " + wfsEvent.getEventId());
        }
    }
}
